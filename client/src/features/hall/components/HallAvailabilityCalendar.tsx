                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                        <div key={day} className={`
                            min-h-[70px] border rounded-lg p-1.5 transition-all relative group flex flex-col
                        ...
                        `}>
                            ...
                            <div className="mt-1 space-y-0.5 overflow-hidden">
                                {dayBookings.map((b: any) => (
                                    <div key={b.id} className="text-[10px] leading-tight bg-blue-100 text-blue-700 px-1 py-0.5 rounded truncate" title={`${b.session} • ${b.guest.name}`}>
                                        {b.session === 'FULL_DAY' ? 'Full' : b.session.substring(0, 1) + b.session.substring(1).toLowerCase()} • {b.guest.name.split(' ')[0]}
                                    </div>
                                ))}
                            </div>
                        </div>
